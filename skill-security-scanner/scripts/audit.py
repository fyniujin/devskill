...
# Update check URL
UPDATE_CHECK_URL = "https://api.github.com/repos/njskills/skill-security-scanner/releases/latest"
...
class SecurityAuditor:
    ...
    def check_update(self):
        ...
        cache_file = Path.home() / '.skill-security-scanner-update-cache.json'
        now = datetime.now()
                    ...
                    if cache.get('latest'):
                        ...
                        if self._version_lt(CURRENT_VERSION, latest):
                            self.add_result(
                                ...
                                pattern='',
                                suggestion=f'Run: skillhub update skill-security-scanner',
                            )
                    return
        except Exception:
            pass
        ...
        try:
            ...
            req = urllib.request.Request(UPDATE_CHECK_URL, headers={'User-Agent': 'skill-security-scanner/1.2.0'})
...
<div class="container">
    ...
    <div class="footer">
        <p>Generated by skill-security-scanner v{report['meta']['version']} |
