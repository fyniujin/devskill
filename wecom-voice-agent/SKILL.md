...
### 能力边界
1. **本技能仅支持企业微信智能机器人场景**，不支持主动外呼、电话拨打或短信发送
...
1. **被动响应**：只处理用户主动发送的语音消息，不主动拨打/发送
2. **意图理解**：将语音转写后的文本解析为结构化意图
3. **任务执行**：调用相应的能力模块完成用户请求
4. **友好回复**：根据用户偏好返回文字或语音消息
...
| 意图类型 | 触发关键词 | 处理方式 |
|---------|-----------|---------|
| `query_schedule` | 日程、会议、安排、行程 | 调用企业微信日程 skill |
| `create_todo` | 提醒、待办、任务、别忘了 | 调用企业微信待办 skill |
| `query_weather` | 天气、气温、下雨、穿什么 | 调用天气查询模块 |
| `send_message` | 发消息、告诉、通知、转发 | 调用企业微信消息 skill |
| `help` | 帮助、能做什么、怎么用 | 返回帮助信息 |
| `exit_voice` | 退出、不用了、谢谢、结束 | 切换到文字模式 |
...
> ⚠️ **重要说明**：Phase 1 提供**会话管理基础设施**（`session_manager.py`），
> 实际技能调用（如日程查询、待办创建）需通过 WorkBuddy Agent 的函数调用实现。
> `session_manager.py` 负责维护对话上下文和历史消息压缩。

**上下文保持规则**：
- 同一用户（同一 `msgid` 前缀）连续消息视为一轮对话
- 单轮对话最多保留 **5 条消息**（3条用户 + 2条助手）
- 超过5条后自动压缩：保留第一条用户消息 + 最近2条消息
- **对话超时**：用户连续 60 秒无新消息则自动结束上下文
...
| 错误类型 | 检查方式 | 处理建议 |
|---------|---------|---------|
| 语音转写失败 | `voice.content` 为空或乱码 | 请求用户重新发送语音 |
| API 调用失败 | HTTP 4xx/5xx 错误码 | 降级为文字回复，提示稍后重试 |
| 意图识别失败 | 关键词匹配度 < 0.3 | 主动询问用户意图，提供选项菜单 |
| TTS 合成失败 | 语音文件生成超时 | 改为文字回复，提示"语音播报暂时不可用" |
| 上下文过期 | 对话间隔 > 60秒 | 自动开始新对话，无需告知用户 |
...
如需更多方言（四川话、河南话等），可在 Phase 2 中接入讯飞或火山 ASR。

### Q3：语音消息长度有限制吗？
**A**：企业微信智能机器人接收的语音消息通常限制在 60 秒以内。
如需处理更长的录音，请使用企业微信的「文件上传」功能，Phase 2 将支持长语音转写。
...
### Q5：支持群聊吗？
**A**：Phase 1 仅支持单聊（`chattype: single`），以确保语音转写准确率和隐私安全。
群聊支持将在 Phase 2 中评估后决定。
