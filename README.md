...
| multi-agent-orchestrator | v5.2.0 | 支持多Agent流水线编排（采集→分析→报告），基于DAG调度实现跨技能状态共享、错误重断点续传、执行报告生成、HTML甘特图可视化、人工审批节点（含超时策略）、历史执行对比、硬件自适应参数和版本更新提醒。v5.2新增统一可视化命令（visualize --format md|html|both）、Self-Improving循环（evaluate节点质量评估+自动重试）、节点级成本追踪（token/cost聚合）。AI即编排器，脚本提供基础设施。 |
