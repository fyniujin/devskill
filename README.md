...
| kingdoc | v3.6.0 | 金山文档 AI 协作助手 — 7 品类在线文档全生命周期管理 （文档/电子表格/演示文稿/多维表格/收集表/可视化/附件）， 深度直连金山文档（WPS）开放平台原生 API，覆盖腾讯文档全部能力 + 金山独有 14 项增强 （回收站、版本历史、格式转换、纯文本提取、本地 Tesseract OCR、通知推送、Webhook、 批量任务、政企合规、硬件自适应性能、WPS AI 能力、协同冲突解决、文档合规检查、 实时协同编辑、文档对比、WPS AI 深度集成、模板市场）。文字/演示/可视化采用"本地生成→上传覆盖"， 电子表格/多维表格采用 API 精细编辑。本地生成、OCR、硬件画像、WPS AI 等能力零密钥可用。 |
