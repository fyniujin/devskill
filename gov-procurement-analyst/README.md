...
面向投标人（供应商）、招标代理、采购单位的**政府采购全流程智能分析助手**，覆盖 19 大模块（含 AI 投标书撰写引擎、竞争对手深度画像、评分预测模型、围标串标检测增强、多省政策差异对比、投标知识库积累），增值功能：智能洞察引擎、投标实操指引、政策影响评估。AI 投标书撰写引擎已从「模板填空」升级为「智能生成 + 人工审核」；围标串标检测从基础规则升级为 NetworkX 多跳图分析+可解释性预警；多省政策差异对比帮助跨省投标人快速识别政策差异；投标知识库让每次投标经验可复用、可检索、可分享。
