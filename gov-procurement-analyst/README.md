...
面向投标人（供应商）、招标代理、采购单位的**政府采购全流程智能分析助手**，覆盖 19 大模块 + 5 个全局组件（含 AI 投标书撰写引擎、竞争对手深度画像、评分预测模型、围标串标检测增强、多省政策差异对比、投标知识库积累、采集配额仪表盘、意图路由器、数据飞轮引擎、**标书协作引擎**），增值功能：智能洞察引擎、投标实操指引、政策影响评估。v5.2.0 起新增标书协作引擎（章节任务拆分+文件哈希锁防并发+合并审校全文一致性检查+资质OCR离线化PaddleOCR+证书版式库）、合规审计双档合并（基础9项+NetworkX图分析一次触发两档输出）、移动盯标推送（关注词命中企微/钉钉卡片推送+三级提醒72/24/6小时）、语音输入指令（口语化指令解析服务外勤场景）。
