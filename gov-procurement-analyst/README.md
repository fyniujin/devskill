...
面向投标人（供应商）、招标代理、采购单位的**政府采购全流程智能分析助手**，覆盖 19 大模块 + 3 个全局组件（含 AI 投标书撰写引擎、竞争对手深度画像、评分预测模型、围标串标检测增强、多省政策差异对比、投标知识库积累、采集配额仪表盘、意图路由器），增值功能：智能洞察引擎、投标实操指引、政策影响评估。v5.0.0 起模块1「商机发现」升级为生产级爬虫（requests+bs4 真实采集 ccgp.gov.cn + 31省分网），新增采集配额仪表盘（内联SVG实时计数+配额告警），新增意图路由器全局架构（L1精确+L2同义词+L3上下文三级消歧，大幅降低误触发）。
