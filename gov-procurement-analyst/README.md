...
面向投标人（供应商）、招标代理、采购单位的**政府采购全流程智能分析助手**，覆盖 19 大模块 + 6 个全局组件（含 AI 投标书撰写引擎、竞争对手深度画像、评分预测模型、围标串标检测增强、多省政策差异对比、投标知识库积累、采集配额仪表盘、意图路由器、数据飞轮引擎、**标书协作引擎**、**竞对战略分析**），增值功能：智能洞察引擎、投标实操指引、政策影响评估。v5.3.0 起新增竞对战略分析（画像→报价策略→评分模拟→中标概率一键完整链路）、评分预测实测数据驱动（特征工程+numpy线性/逻辑回归+样本量不足降级+XGBoost可选增强）、报价策略沙盘（遍历报价区间模拟综合得分曲线+竞对扰动敏感性分析）、废标原因库与自检清单（12类废标原因分类+标书完成逐项打钩拦截报告）。
...
- **paddleocr + paddlepaddle**（本地离线OCR，v5.2 新增，用于资质OCR离线化，替换在线OCR）
- **networkx**（v5.2 起升级为推荐安装，围标串标多跳图分析+合规审计双档合并图分析档，无则降级为两跳规则匹配+标注"图分析未运行"）

**内置（无需安装）**：
- sqlite3（Python 标准库，竞对画像 + 评分预测 + 围标图分析 + 政策数据库 + 投标知识库存储）
- numpy（v5.3 新增，Python 标准安装，评分预测实测数据驱动 — 线性回归/逻辑回归，零依赖实现）

**可选增强**：
- xgboost（v5.3 新增，评分预测可选增强依赖，安装即启用双列对比 — numpy预测 vs XGBoost预测）

## 版本历史

- **v5.3.0 (2026-09-20)**：新增竞对战略分析（画像→报价策略→评分模拟→中标概率一键完整链路，模块17+18合并）；评分预测实测数据驱动（特征工程+numpy线性/逻辑回归+样本量不足50降级规则分+XGBoost可选增强）；新增报价策略沙盘（遍历报价区间模拟综合得分曲线+竞对扰动敏感性分析）；新增废标原因库与自检清单（12类废标原因分类+逐项打钩拦截报告）；新增触发词4条+FAQ Q34-Q38
- **v5.2.0 (2026-09-03)**：新增标书协作引擎（章节任务拆分+文件哈希锁防并发+合并审校全文一致性检查+资质OCR离线化PaddleOCR+证书版式库）；合规审计双档合并（模块5基础9项+模块19图分析一次触发两档输出，NetworkX由可选升级为推荐安装）；移动盯标推送（关注词/区域/金额段配置+企微/钉钉webhook卡片推送+去抖规则）；三级提醒（投标截止前72/24/6小时分级提醒）；语音输入指令（口语化指令解析服务外勤场景，意图路由器L2/L3扩展）；新增触发词7条+FAQ Q29-Q33
- v5.1.0 (2026-08-24)：新增数据飞轮引擎（中标结果回流建立第一方数据底座+行业价格指数汇聚分散中标数据为可比指标+复盘链自动串联让每次投标经验自动沉淀+竞对实时监控把静态画像升级为动态预警+爬虫失效自愈降低31省分网维护成本）；SQLite星型模型扩展（事实表=项目+维度表=行业/区域/时间/竞对）
- v5.0.0 (2026-08-16)：模块1升级为生产级爬虫（requests+bs4真实采集ccgp.gov.cn+31省分网）；新增采集配额仪表盘（内联SVG实时计数+配额告警+域名分布）；新增意图路由器全局架构；模块1双选择器自愈机制（主备CSS路径自动切换+哈希监测+降权重调度）；模块6复盘链自动串联（中标→复盘→入库一条链）；模块17竞对实时监控（关注清单+变更检测+企微/钉钉推送）
