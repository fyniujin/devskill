...
- **📊 评分可视化**：每项扣分透明化（严重-15/中等-8/一般-3），评分依据一目了然
- **🪄 首次使用向导**：引导选择运行模式（Ollama 本地 / OpenAI API / 规则引擎）
- **🦙 Ollama 一键安装**：内建本地模型安装向导，无需手动配置
- **📈 实时进度显示**：每步有状态图标（⏳执行中 / ✅完成 / ⚠️警告）
- **🚀 更强易用性**：安装即用，无需配置即可使用基础审查功能
- **📍 风险定位更准确**：页码+条款编号双重定位，不显示"第?页"
- **📦 标准化打包**：新增 pyproject.toml，支持 pip install -e . 安装
...
5. **文件安全校验**：检查魔术字节、文件类型一致性、50MB 大小限制
6. **YAML 安全加载**：使用 `yaml.safe_load` 防止反序列化攻击
7. **日志脱敏**：不输出完整文件路径和合同原文到日志
8. **个人信息保护**：报告中的身份证号、手机号等信息自动脱敏
...
### 问题：报告中"第?页"显示
- **解决**：某些 PDF 文件不包含页码信息。v2.3 会尽量从页眉页脚识别页码。
...
- `scripts/extract_text.py` — 多格式文本提取（带安全校验、魔术字节验证）
...
- `scripts/main.py` — 主入口（首次向导 + Ollama 安装 + 完整进度）
