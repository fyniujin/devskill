...
> **v3.7.0**：多维表格视图增强（看板/日历/甘特图，对标 Airtable）· 手写/公式识别 OCR（教育场景，LaTeX/MathML 输出）· 历史管理（回收站+版本历史统一入口）· 深度直连 WPS 开放平台原生 API · 文档模板市场（行业模板库一键复用）· WPS AI 深度集成（段落级改写/总结/续写）· 实时协同编辑（序列 CRDT，多人无冲突）· 文档对比（差异高亮/变更摘要/导出）· 文档内容合规检查（敏感词/泄露/格式/密级）· 协同编辑冲突解决（检测/合并/diff/解决）· WPS AI 能力（写作/分析/PPT/阅读）· 硬件自适应性能 · 免密钥本地能力。

## ✨ 特性

- 📝 **8 品类全覆盖** — 文档/表格/演示/多维表格/收集表/可视化/历史管理/附件
- 📊 **多维表格视图增强** — 看板/日历/甘特图，对标 Airtable，本地渲染
- ✍️ **手写/公式识别** — 教育场景，LaTeX/MathML 输出，强制本地 Tesseract
- 🕐 **历史管理** — 回收站+版本历史统一入口（list/restore/destroy）
