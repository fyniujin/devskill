...
class BilibiliAdapter(PlatformAdapter):
    ...
    BILIBILI_PATTERNS = [
        ...
        r'm\.bilibili\.com/video/(BV[A-Za-z0-9]+)',
    ]
    ...
    def parse_link(self, link: str) -> Optional[str]:
        ...
        for pattern in self.BILIBILI_PATTERNS:
            ...
            if match:
                ...
                if video_id.startswith("BV"):
                    logger.info(f"解析B站视频ID: {video_id}")
                    return video_id
                else:
                    # AV 号转 BV 号（简化处理，返回 av 号）
                    logger.info(f"解析B站视频ID: av{video_id}")
        ...
        logger.warning(f"无法解析B站链接: {link}")
        return None
    ...
    def download_video(self, video_id: str, output_dir: str) -> Optional[str]:
        ...
        if video_id.startswith("BV"):
            ...
            video_url = f"https://www.bilibili.com/video/av{video_id[2:]}"
        else:
            video_url = f"https://www.bilibili.com/video/{video_id}"
        
        output_path = os.path.join(output_dir, f"bilibili_{video_id}.mp4")
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(output_path):
                logger.info(f"B站视频下载成功: {output_path}")
                return output_path
            else:
                logger.error(f"B站视频下载失败: {result.stderr.decode()}")
                return None
    ...
    def extract_metadata(self, video_id: str) -> PlatformMetadata:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                metadata.collect_count = data.get("view_count", 0)  # B站收藏数需额外API
    ...
    def _analyze_completion_factors(
        ...
        if 30 <= duration <= 180:
            ...
            factors["video_length_score"] = 85
        elif duration <= 300:
            factors["video_length_score"] = 75
        else:
            factors["video_length_score"] = 55
        ...
        if transcript:
            ...
            cta_keywords = ["关注", "点赞", "投币", "收藏", "转发", "三连", "评论区"]
            factors["has_call_to_action"] = any(kw in full_text for kw in cta_keywords)
        
        if transcript:
            full_text = transcript.get("text", "")
            suspense_keywords = ["但是", "然而", "竟然", "原来", "没想到", "秘密", "注意"]
    ...
    def _analyze_ecommerce(self, transcript: Dict = None) -> Dict:
        ...
        cta_patterns = ["点击链接", "评论区", "私信", "关注我", "下单", "小黄车"]
