...
class WechatVideoAdapter(PlatformAdapter):
    ...
    注意：视频号链接多为分享链接，解析难度较高
    """
    
    # 视频号链接正则
    WECHAT_PATTERNS = [
        r'weixin\.qq\.com/\?.*(?:v=|video_id=)([A-Za-z0-9]+)',
        r'channels\.weixin\.qq\.com/([A-Za-z0-9]+)',
        r'video\.weixin\.qq\.com/([A-Za-z0-9]+)',
    ]
    ...
    def parse_link(self, link: str) -> Optional[str]:
        ...
        for pattern in self.WECHAT_PATTERNS:
            ...
            if match:
                ...
                logger.info(f"解析视频号视频ID: {video_id}")
                return video_id
        
        # 视频号链接通常嵌在分享文本中，尝试提取 URL 参数
        if "weixin.qq.com" in link or "channels" in link:
            # 尝试从完整 URL 提取
            try:
                from urllib.parse import urlparse, parse_qs
                parsed = urlparse(link)
                qs = parse_qs(parsed.query)
                for key in ["v", "video_id", "id"]:
                    if key in qs:
                        return qs[key][0]
            except Exception:
                pass
        
        logger.warning(f"无法解析视频号链接: {link}")
        return None
    ...
    def download_video(self, video_id: str, output_dir: str) -> Optional[str]:
        ...
        video_url = f"https://video.weixin.qq.com/?v={video_id}"
        output_path = os.path.join(output_dir, f"wechat_video_{video_id}.mp4")
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(output_path):
                logger.info(f"视频号视频下载成功: {output_path}")
                return output_path
            else:
                logger.error(f"视频号视频下载失败: {result.stderr.decode()}")
                return None
