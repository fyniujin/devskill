"""StepFun (阶跃星辰) model API adapter."""
...
class StepFunAdapter(BaseAdapter):
    """StepFun (阶跃星辰) Step series adapter."""

    provider_name = "stepfun"
    default_model = "step-1-200k"
    ...
    def chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            raise RuntimeError(
                f"[StepFun] HTTP {e.code}: {body[:200]}。请检查 api_key 是否正确。"
            ) from e
        except Exception as e:
            raise RuntimeError(f"[StepFun] 网络错误: {e}") from e

        usage = raw.get("usage", {})
        try:
            text = raw["choices"][0]["message"]["content"]
        except (KeyError, IndexError) as e:
            raise RuntimeError(f"[StepFun] 解析响应失败: {raw}") from e
    ...
    def stream_chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[StepFun] 流式 HTTP {e.code}: {body[:200]}") from e
