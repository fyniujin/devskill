"""Baichuan AI model adapter - https://api.baichuan-ai.com"""
...
class BaichuanAdapter(BaseAdapter):
    ...
    def chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            raise RuntimeError(
                f"[Baichuan] HTTP {e.code}: {body[:200]}。请检查 api_key 是否正确。"
            ) from e
        except Exception as e:
            raise RuntimeError(f"[Baichuan] 网络错误: {e}") from e

        usage = raw.get("usage", {})
        try:
            text = raw["choices"][0]["message"]["content"]
        except (KeyError, IndexError) as e:
            raise RuntimeError(f"[Baichuan] 解析响应失败: {raw}") from e
    ...
    def stream_chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[Baichuan] 流式 HTTP {e.code}: {body[:200]}") from e
