"""LingYi (Zero2AI / Yi) model adapter - https://api.lingyi.cn"""
...
# LingYi API (OpenAI-compatible endpoint)
API_URL = "https://api.lingyi.cn/v1/chat/completions"


class LingYiAdapter(BaseAdapter):
    """LingYi (Zero2AI) Yi series adapter."""

    provider_name = "lingyi"
    ...
    def chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            raise RuntimeError(
                f"[LingYi] HTTP {e.code}: {body[:200]}。请检查 api_key 是否正确。"
            ) from e
        except Exception as e:
            raise RuntimeError(f"[LingYi] 网络错误: {e}") from e

        usage = raw.get("usage", {})
        try:
            text = raw["choices"][0]["message"]["content"]
        except (KeyError, IndexError) as e:
            raise RuntimeError(f"[LingYi] 解析响应失败: {raw}") from e
    ...
    def stream_chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[LingYi] 流式 HTTP {e.code}: {body[:200]}") from e
