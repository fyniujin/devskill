"""ZhiPu (智谱 GLM) model adapter for llm-core shared kernel."""
...
from ..types import (
    ...
    EmbeddingResult, RerankResult, now_ms,
)
...
class ZhiPuAdapter(BaseAdapter):
    ...
    def chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            raise RuntimeError(
                f"[ZhiPu] HTTP {e.code}: {body[:200]}。请检查 api_key 是否正确。"
            ) from e
        except Exception as e:
            raise RuntimeError(f"[ZhiPu] 网络错误: {e}") from e
    ...
    def stream_chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[ZhiPu] 流式 HTTP {e.code}: {body[:200]}") from e
    ...
    def embed_text(self, texts: List[str], *,
        ...
        """ZhiPu supports embeddings via embedding-2 model."""
        m = model or "embedding-2"
        start = self._now_ms()
        payload = {"model": m, "input": texts}
        req = urllib.request.Request(
            EMBED_URL, data=json.dumps(payload).encode("utf-8"),
            headers=self._build_headers(), method="POST",
        )
        try:
            with urllib.request.urlopen(req, timeout=60) as resp:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[ZhiPu] embed_text HTTP {e.code}: {body[:200]}") from e
        except Exception as e:
            raise RuntimeError(f"[ZhiPu] embed_text 网络错误: {e}") from e

        embeddings = [item.get("embedding", []) for item in raw.get("data", [])]
        return EmbeddingResult(
            provider=self.provider_name, model=m,
            embeddings=embeddings, usage=raw.get("usage", {}),
            duration_ms=self._now_ms() - start,
        )

    def rerank(self, query: str, documents: List[str], *,
               model: Optional[str] = None, **kwargs: Any) -> RerankResult:
        """ZhiPu supports reranking via rerank model."""
        m = model or "rerank"
        start = self._now_ms()
        payload = {"model": m, "query": query, "documents": documents}
        req = urllib.request.Request(
            RERANK_URL, data=json.dumps(payload).encode("utf-8"),
            headers=self._build_headers(), method="POST",
        )
        try:
            with urllib.request.urlopen(req, timeout=60) as resp:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[ZhiPu] rerank HTTP {e.code}: {body[:200]}") from e
        except Exception as e:
            raise RuntimeError(f"[ZhiPu] rerank 网络错误: {e}") from e

        scores = [item.get("relevance_score", 0.0) for item in raw.get("results", [])]
        return RerankResult(
            provider=self.provider_name, model=m,
            scores=scores, usage=raw.get("usage", {}),
            duration_ms=self._now_ms() - start,
        )
