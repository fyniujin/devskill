"""TongYi (Alibaba DashScope) model adapter for llm-core shared kernel."""
...
class TongYiAdapter(BaseAdapter):
    ...
    def chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            raise RuntimeError(
                f"[TongYi] HTTP {e.code}: {body[:200]}。请检查 api_key 是否正确。"
            ) from e
        except Exception as e:
            raise RuntimeError(f"[TongYi] 网络错误: {e}") from e
    ...
    def stream_chat(self, messages: List[ChatMessage], *,
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[TongYi] 流式 HTTP {e.code}: {body[:200]}") from e
    ...
    def embed_text(self, texts: List[str], *,
        ...
        """TongYi supports embeddings via text-embedding model."""
        m = model or "text-embedding-v2"
        start = self._now_ms()
        payload = {"model": m, "input": {"texts": texts}}
        req = urllib.request.Request(
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"[TongYi] embed_text HTTP {e.code}: {body[:200]}") from e
        except Exception as e:
            raise RuntimeError(f"[TongYi] embed_text 网络错误: {e}") from e

        embeddings = []
        for item in raw.get("output", {}).get("embeddings", []):
