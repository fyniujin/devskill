...
def main():
    ...
    p.add_argument("--file", required=True)
    p.add_argument("--sheet", default="Sheet1")
    p.add_argument("--sort-json", required=True, help='JSON: [{"column":"金额","ascending":false}]')

    p = sub.add_parser("filter")
    p.add_argument("--file", required=True)
    p.add_argument("--sheet", default="Sheet1")
    p.add_argument("--filter-json", required=True, help='JSON: [{"column":"金额","op":">","value":"1000"}]')
    ...
    if args.command == "create":
        ...
        sorts = json.loads(args.sort_json)
        r = call_worker("sort_excel", {
            "filepath": args.file, "sheet": args.sheet, "sorts": sorts
        })
    elif args.command == "filter":
        conds = json.loads(args.filter_json)
        r = call_worker("filter_excel", {
            "filepath": args.file, "sheet": args.sheet, "conditions": conds
        })
